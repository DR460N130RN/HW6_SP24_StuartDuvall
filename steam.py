...
class steam():
    ...
    def calc(self):
        ...
        data = np.array([
            ...
            # Add all the data points from the provided steam properties table here
            # Format: [T, p, h, s]
        ])
        ...
        hf = float(griddata(ps, hs, Pbar))
        hg = float(griddata(ps, hs, Pbar))  # Use the same hs data for hg
        sf = float(griddata(ps, ss, Pbar))
        sg = float(griddata(ps, ss, Pbar))  # Use the same ss data for sg
        ...
        if self.T is not None:
            if self.T > Tsat:  # interpolate with griddata
                ...
                self.h = griddata((ts, ps), hs, (self.T, self.p))
                self.s = griddata((ts, ps), ss, (self.T, self.p))
                self.x = 1.0
            ...
            self.v = 1.0 / (vf + self.x * (vg - vf))  # Use reciprocal of specific volume for saturated liquid/vapor
        elif self.h != None:
            ...
            if self.x <= 1.0:  # manual interpolation
                ...
                self.region = 'Superheated'
                self.T = griddata((hs, ps), ts, (self.h, self.p))
                self.s = griddata((hs, ps), ss, (self.h, self.p))
        elif self.s != None:
            ...
            if self.x <= 1.0:  # manual interpolation
                ...
                self.region = 'Superheated'
                self.T = griddata((ss, ps), ts, (self.s, self.p))
                self.h = griddata((ss, ps), hs, (self.s, self.p))
        # endregion
